...
export class ProfileService {
  ...
  getProfile(emailID:any) : Observable<Profiler> {
      let token   = localStorage.getItem('auth_token');
      let headers = new Headers({ 'Content-Type': 'application/json', 'x-access-token': token });
      let options = new RequestOptions({ headers: headers });
      let Url = this.global.GlobalUrl+'/company/' + emailID;
      return this.http.get(Url, options)
      .map((res:Response) => res.json())      
      .catch((error:any) => Observable.throw(error.json().error || 'Data not found!'));
  }

  updateinsertProfile(body: Object): Observable<Object[]> {
    let bodyString = JSON.stringify(body); // Stringify payload
    let headers      = new Headers({ 'Content-Type': 'application/json' }); // ... Set content type to JSON
    let options       = new RequestOptions({ headers: headers }); // Create a request option
    let Url = this.global.GlobalUrl+'/company/update';
    return this.http.put(Url, body, options) // ...using post request
      .map((res:Response) => {
        console.log('updsert profile', res.json());
        res.json()}
